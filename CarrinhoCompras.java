...
public class CarrinhoCompras {
    ...
    public void adicionaraoCarrinho(Jogo jogo) {
        jogos.add(jogo);
        System.out.println("O model.Jogo " + jogo.getNome() + " foi adicionado ao carrinho");
    }

    public void removerDoCarrinho(Jogo jogo) {
        jogos.remove(jogo);
        System.out.println("O model.Jogo " + jogo.getNome() + " foi removido do carrinho");
    }
    ...
    public void removerJogosDoArquivo(List<Jogo> jogosParaRemover) {
        try {
            ...
            List<String> linhasAtualizadas = linhas.stream()
                    .filter(linha -> jogosParaRemover.stream().noneMatch(jogo -> linha.contains(jogo.getNome())))
