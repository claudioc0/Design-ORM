...
public class CarrinhoComprasGUI extends JFrame {
    ...
    private void exibirJogosNoCarrinho(JPanel centerPanel) {
        ...
        for (Jogo jogo : jogosAnunciados) {
            jogosAnunciadosNomes.add(jogo.getNome());
        }

        for (Jogo jogo : jogosNoCarrinho) {
            // Verificar se o jogo está nos jogos anunciados
            if (!jogosAnunciadosNomes.contains(jogo.getNome())) {
            ...
            JLabel nomeLabel = new JLabel("Nome: " + jogo.getNome());
            JLabel generoLabel = new JLabel("Gênero: " + jogo.getGenero());
            JLabel precoLabel = new JLabel("Preço: R$" + jogo.getPreco());
    ...
    private void removerJogoDoCarrinho(Jogo jogo) {
        ...
        JOptionPane.showMessageDialog(this, "model.Jogo removido do carrinho!");
    }
